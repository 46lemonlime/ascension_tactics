...
export function buildDarkEldarFigure(
  ...
  const obsidianMat = getCachedStandardMaterial({ color: 0x042f2e, roughness: 0.3, metalness: 0.85 });
  const toxicGlowMat = getCachedBasicMaterial({ color: 0x10b981 });
  ...
  const gunMetalMat = getCachedStandardMaterial({ color: 0x1e293b, roughness: 0.5, metalness: 0.75 });
  const flayedMat = getCachedStandardMaterial({ color: 0x701a75, roughness: 0.85 });

  if (uType === 'de_archon') {
    figBody.scale.set(1.25, 1.25, 1.25);

    const legGeo = getCachedCylinder(0.08, 0.11, 0.6, 14);
    ...
    const torso = new THREE.Mesh(getCachedBox(0.44, 0.58, 0.28), obsidianMat);
    torso.position.y = 0.88;
    figBody.add(torso);

    const trophyPole1 = new THREE.Mesh(getCachedCylinder(0.02, 0.02, 0.9, 10), gunMetalMat);
    trophyPole1.position.set(-0.18, 1.35, -0.16);
    trophyPole1.rotation.z = 0.2;
    figBody.add(trophyPole1);
    const trophyPole2 = new THREE.Mesh(getCachedCylinder(0.02, 0.02, 0.9, 10), gunMetalMat);
    trophyPole2.position.set(0.18, 1.35, -0.16);
    trophyPole2.rotation.z = -0.2;
    figBody.add(trophyPole2);
    const bladeTrophy1 = new THREE.Mesh(getCachedCone(0.05, 0.28, 10), bladeMat);
    bladeTrophy1.position.set(-0.25, 1.7, -0.16);
    figBody.add(bladeTrophy1);
    const bladeTrophy2 = new THREE.Mesh(getCachedCone(0.05, 0.28, 10), bladeMat);
    bladeTrophy2.position.set(0.25, 1.7, -0.16);
    figBody.add(bladeTrophy2);

    const cape = new THREE.Mesh(getCachedBox(0.52, 0.95, 0.04), flayedMat);
    cape.position.set(0, 0.78, -0.18);
    cape.rotation.x = -0.12;
    figBody.add(cape);

    const collar = new THREE.Mesh(getCachedBox(0.38, 0.25, 0.04), trimMat);
    collar.position.set(0, 1.22, -0.1);
    figBody.add(collar);

    const head = new THREE.Mesh(getCachedBox(0.22, 0.28, 0.24), obsidianMat);
    head.position.y = 1.32;
    figBody.add(head);

    const visor = new THREE.Mesh(getCachedBox(0.16, 0.04, 0.04), toxicGlowMat);
    visor.position.set(0, 1.34, 0.13);
    figBody.add(visor);

    const swordBlade = new THREE.Mesh(getCachedBox(0.05, 0.95, 0.14), bladeMat);
    swordBlade.position.set(0.42, 1.15, 0.35);
    swordBlade.rotation.x = -0.45;
    figBody.add(swordBlade);

    const pistol = new THREE.Mesh(getCachedBox(0.09, 0.12, 0.42), gunMetalMat);
    pistol.position.set(-0.36, 0.85, 0.28);
    figBody.add(pistol);
  } else if (uType === 'de_incubi') {
    figBody.scale.set(1.1, 1.1, 1.1);

    const legGeo = getCachedCylinder(0.09, 0.12, 0.58, 14);
    const leftLeg = new THREE.Mesh(legGeo, obsidianMat);
    ...
    const torso = new THREE.Mesh(getCachedBox(0.46, 0.56, 0.28), obsidianMat);
    torso.position.y = 0.85;
    figBody.add(torso);

    const head = new THREE.Mesh(getCachedBox(0.24, 0.28, 0.26), obsidianMat);
    head.position.y = 1.25;
    figBody.add(head);

    const hornGeo = getCachedCone(0.05, 0.42, 12);
    ...
    const eyes = new THREE.Mesh(getCachedBox(0.16, 0.04, 0.04), toxicGlowMat);
    eyes.position.set(0, 1.26, 0.14);
    figBody.add(eyes);

    const shaft = new THREE.Mesh(getCachedCylinder(0.03, 0.03, 1.4, 12), gunMetalMat);
    shaft.position.set(0.32, 0.95, 0.35);
    shaft.rotation.x = -0.35;
    figBody.add(shaft);
    const klaiveBlade = new THREE.Mesh(getCachedBox(0.06, 0.85, 0.22), bladeMat);
    ...
    const legGeo = getCachedCylinder(0.08, 0.1, 0.58, 14);
    const leftLeg = new THREE.Mesh(legGeo, obsidianMat);
    ...
    const torso = new THREE.Mesh(getCachedBox(0.42, 0.52, 0.26), obsidianMat);
    torso.position.y = 0.82;
    figBody.add(torso);

    const wingGeo = getCachedBox(0.85, 0.35, 0.03);
    ...
    const head = new THREE.Mesh(getCachedBox(0.22, 0.26, 0.24), obsidianMat);
    head.position.y = 1.22;
    figBody.add(head);
    const visor = new THREE.Mesh(getCachedBox(0.14, 0.04, 0.04), toxicGlowMat);
    visor.position.set(0, 1.23, 0.13);
    figBody.add(visor);

    const lance = new THREE.Mesh(getCachedBox(0.14, 0.18, 1.35), gunMetalMat);
    lance.position.set(0.28, 0.85, 0.55);
    figBody.add(lance);
    const lanceTip = new THREE.Mesh(getCachedCone(0.06, 0.35, 10), bladeMat);
    ...
    const hull = new THREE.Mesh(getCachedBox(0.85, 0.28, 1.8), obsidianMat);
    hull.position.set(0, 0.35, 0);
    figBody.add(hull);
    root.userData.leftLeg = hull;
    root.userData.rightLeg = hull;

    const prowBlade = new THREE.Mesh(getCachedBox(0.06, 0.35, 0.65), bladeMat);
    prowBlade.position.set(0, 0.35, 1.15);
    prowBlade.rotation.x = 0.4;
    figBody.add(prowBlade);

    const sailGeo = getCachedBox(0.04, 1.1, 0.55);
    const sail = new THREE.Mesh(sailGeo, trimMat);
    sail.position.set(0, 1.05, -0.65);
    sail.rotation.x = -0.25;
    figBody.add(sail);

    const prowLance = new THREE.Mesh(getCachedCylinder(0.04, 0.05, 1.25, 14), gunMetalMat);
    prowLance.position.set(0, 0.55, 0.95);
    prowLance.rotation.x = Math.PI / 2;
    figBody.add(prowLance);

    const lLance = new THREE.Mesh(getCachedCylinder(0.04, 0.05, 1.15, 14), gunMetalMat);
    lLance.position.set(-0.55, 0.48, 0.25);
    lLance.rotation.x = Math.PI / 2;
    figBody.add(lLance);
    const rLance = new THREE.Mesh(getCachedCylinder(0.04, 0.05, 1.15, 14), gunMetalMat);
    rLance.position.set(0.55, 0.48, 0.25);
    rLance.rotation.x = Math.PI / 2;
    figBody.add(rLance);

    const engineL = new THREE.Mesh(getCachedSphere(0.12, 12, 12), toxicGlowMat);
    engineL.position.set(-0.35, 0.32, -0.92);
    figBody.add(engineL);
    const engineR = new THREE.Mesh(getCachedSphere(0.12, 12, 12), toxicGlowMat);
    ...
    const torso = new THREE.Mesh(getCachedBox(0.38, 0.48, 0.24), obsidianMat);
    torso.position.y = 0.75;
    figBody.add(torso);

    const head = new THREE.Mesh(getCachedBox(0.2, 0.26, 0.22), obsidianMat);
    head.position.y = 1.1;
    figBody.add(head);

    const crest = new THREE.Mesh(getCachedCone(0.08, 0.35, 10), trimMat);
    ...
    const rifle = new THREE.Mesh(getCachedBox(0.1, 0.14, 0.65), gunMetalMat);
    rifle.position.set(0.24, 0.68, 0.35);
    figBody.add(rifle);
    const bayonet = new THREE.Mesh(getCachedBox(0.02, 0.08, 0.22), bladeMat);
