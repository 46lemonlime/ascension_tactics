...
export function buildNecronFigure(
  ...
  const necroMat = getCachedStandardMaterial({ color: 0x334155, roughness: 0.35, metalness: 0.85 });
  const gaussGlowMat = getCachedBasicMaterial({ color: 0x22c55e });
  const gaussDarkMat = getCachedStandardMaterial({ color: 0x0f172a, roughness: 0.5, metalness: 0.8 });
  const goldGlyphMat = getCachedStandardMaterial({ color: 0xeab308, roughness: 0.25, metalness: 0.9 });

  if (uType === 'nec_overlord') {
    figBody.scale.set(1.35, 1.35, 1.35);

    const legGeo = getCachedCylinder(0.08, 0.1, 0.65, 14);
    ...
    const torso = new THREE.Mesh(getCachedBox(0.48, 0.65, 0.3), necroMat);
    torso.position.y = 0.9;
    figBody.add(torso);

    const ankh = new THREE.Mesh(getCachedBox(0.24, 0.32, 0.08), goldGlyphMat);
    ankh.position.set(0, 0.92, 0.16);
    figBody.add(ankh);

    const nemesCollar = new THREE.Mesh(getCachedTorus(0.36, 0.08, 10, 20, Math.PI), goldGlyphMat);
    nemesCollar.position.set(0, 1.25, 0.02);
    nemesCollar.rotation.z = Math.PI;
    figBody.add(nemesCollar);

    const head = new THREE.Mesh(getCachedBox(0.24, 0.32, 0.26), necroMat);
    head.position.y = 1.38;
    figBody.add(head);

    const eyeL = new THREE.Mesh(getCachedSphere(0.04, 10, 10), gaussGlowMat);
    eyeL.position.set(-0.06, 1.4, 0.14);
    figBody.add(eyeL);
    const eyeR = new THREE.Mesh(getCachedSphere(0.04, 10, 10), gaussGlowMat);
    eyeR.position.set(0.06, 1.4, 0.14);
    figBody.add(eyeR);

    const cloak = new THREE.Mesh(getCachedBox(0.45, 1.0, 0.04), gaussDarkMat);
    cloak.position.set(0, 0.8, -0.18);
    cloak.rotation.x = -0.1;
    figBody.add(cloak);

    const staff = new THREE.Mesh(getCachedCylinder(0.035, 0.035, 1.7, 14), goldGlyphMat);
    staff.position.set(0.45, 1.05, 0.2);
    figBody.add(staff);
    const scytheBlade = new THREE.Mesh(
      getCachedTorus(0.28, 0.05, 8, 20, Math.PI * 0.75),
      gaussGlowMat
    );
    ...
    const lArm = new THREE.Mesh(getCachedCylinder(0.05, 0.06, 0.5, 12), necroMat);
    lArm.position.set(-0.35, 0.95, 0.2);
    lArm.rotation.x = -0.6;
    figBody.add(lArm);
    const resOrb = new THREE.Mesh(getCachedSphere(0.14, 16, 16), gaussGlowMat);
    ...
    const torso = new THREE.Mesh(getCachedBox(0.52, 0.6, 0.32), necroMat);
    torso.position.y = 0.85;
    figBody.add(torso);

    const shGeo = getCachedCylinder(0.18, 0.22, 0.24, 12);
    ...
    const head = new THREE.Mesh(getCachedBox(0.24, 0.3, 0.26), necroMat);
    head.position.y = 1.3;
    figBody.add(head);
    const visor = new THREE.Mesh(getCachedBox(0.16, 0.05, 0.05), gaussGlowMat);
    visor.position.set(0, 1.32, 0.14);
    figBody.add(visor);

    const blaster = new THREE.Mesh(getCachedBox(0.22, 0.22, 0.85), gaussDarkMat);
    blaster.position.set(0.28, 0.82, 0.45);
    figBody.add(blaster);
    const rod = new THREE.Mesh(getCachedCylinder(0.04, 0.04, 0.6, 14), gaussGlowMat);
    ...
    for (let l = 0; l < 3; l++) {
      ...
      const legUpper = new THREE.Mesh(getCachedBox(0.08, 0.42, 0.1), gaussDarkMat);
      ...
      const legLower = new THREE.Mesh(getCachedBox(0.06, 0.45, 0.08), necroMat);
    ...
    const torso = new THREE.Mesh(getCachedBox(0.56, 0.62, 0.38), necroMat);
    torso.position.set(0, 0.85, 0.08);
    torso.rotation.x = 0.2;
    figBody.add(torso);

    const head = new THREE.Mesh(getCachedBox(0.26, 0.28, 0.3), gaussDarkMat);
    head.position.set(0, 1.18, 0.24);
    figBody.add(head);
    const opticEye = new THREE.Mesh(getCachedSphere(0.08, 14, 14), gaussGlowMat);
    opticEye.position.set(0, 1.18, 0.39);
    figBody.add(opticEye);

    const bladeGeo = getCachedTorus(0.36, 0.06, 8, 24, Math.PI * 0.85);
    ...
    const sledGeo = getCachedCylinder(0.55, 0.42, 0.32, 16);
    ...
    const repulsorGlow = new THREE.Mesh(getCachedCylinder(0.38, 0.38, 0.08, 16), gaussGlowMat);
    ...
    const head = new THREE.Mesh(getCachedBox(0.24, 0.28, 0.26), necroMat);
    head.position.set(0, 1.15, 0.18);
    figBody.add(head);
    const eye = new THREE.Mesh(getCachedSphere(0.06, 12, 12), gaussGlowMat);
    eye.position.set(0, 1.16, 0.31);
    figBody.add(eye);

    const cannon = new THREE.Mesh(getCachedBox(0.26, 0.3, 1.35), gaussDarkMat);
    cannon.position.set(0.32, 0.75, 0.65);
    cannon.rotation.x = -0.05;
    figBody.add(cannon);

    for (let r = 0; r < 2; r++) {
      const rod = new THREE.Mesh(getCachedCylinder(0.04, 0.04, 0.85, 14), gaussGlowMat);
    ...
    for (let l = 0; l < 4; l++) {
      ...
      const upperStilt = new THREE.Mesh(getCachedBox(0.08, 0.85, 0.08), gaussDarkMat);
      ...
      const lowerStilt = new THREE.Mesh(getCachedBox(0.06, 0.95, 0.06), necroMat);
    ...
    const dome = new THREE.Mesh(getCachedSphere(0.45, 16, 16), gaussDarkMat);
    dome.position.set(0, 1.25, 0);
    figBody.add(dome);

    const reactorEye = new THREE.Mesh(getCachedSphere(0.18, 14, 14), gaussGlowMat);
    reactorEye.position.set(0, 1.25, 0.38);
    figBody.add(reactorEye);

    const blasterBase = new THREE.Mesh(getCachedCylinder(0.22, 0.28, 0.45, 14), necroMat);
    blasterBase.position.set(0, 1.7, 0.05);
    figBody.add(blasterBase);

    const blasterBarrel = new THREE.Mesh(getCachedCylinder(0.12, 0.16, 1.85, 16), gaussDarkMat);
    blasterBarrel.position.set(0, 1.82, 0.85);
    blasterBarrel.rotation.x = Math.PI / 2;
    figBody.add(blasterBarrel);

    for (let ring = 0; ring < 3; ring++) {
      const accRing = new THREE.Mesh(getCachedTorus(0.24, 0.04, 10, 20), gaussGlowMat);
    ...
    const legGeo = getCachedCylinder(0.07, 0.09, 0.55, 12);
    const leftLeg = new THREE.Mesh(legGeo, necroMat);
    ...
    const torso = new THREE.Mesh(getCachedBox(0.4, 0.52, 0.26), necroMat);
    torso.position.set(0, 0.78, 0.03);
    torso.rotation.x = 0.1;
    figBody.add(torso);

    const head = new THREE.Mesh(getCachedBox(0.2, 0.26, 0.22), necroMat);
    head.position.set(0, 1.12, 0.12);
    figBody.add(head);
    const eye = new THREE.Mesh(getCachedSphere(0.04, 10, 10), gaussGlowMat);
    eye.position.set(0, 1.14, 0.23);
    figBody.add(eye);

    const flayer = new THREE.Mesh(getCachedBox(0.14, 0.14, 0.7), gaussDarkMat);
    flayer.position.set(0.22, 0.72, 0.35);
    figBody.add(flayer);
    const rod = new THREE.Mesh(getCachedCylinder(0.03, 0.03, 0.45, 12), gaussGlowMat);
